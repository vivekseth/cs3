...
int validateMacAddress(u_char *ipAddress, u_char *macAddress) {
	int i, j;
...
void ARPSpoofingCallback(u_char *user, const struct pcap_pkthdr *h, const u_char *packet) {
	...
	if (!validateMacAddress(sourceProtocolAddress, sourceHardwareAddress)) {
...
int foundAttack = 0;
u_char IP_PORT_SCAN_ATTACKER[4];
...
void portScanAttackCallback(u_char *user, const struct pcap_pkthdr *h, const u_char *packet) {
	...
	u_char *src = (u_char *)&(ip->ip_src);

	// only look at packets to victim
	if (!compareMem(dest, IP_PORT_SCAN_VICTIM, 4)) {
		return;
	}

	// Lazy load port set
	PortSet *portset = NULL;
	if (IPAddrHashTable_read_value(table, dest, (void **)&portset)) {
		portset = PortSet_create();
		IPAddrHashTable_set_value(table, dest, portset);
	}
	...
	if (ip->ip_p == IPPROTO_TCP) {
		...
		PortSet_set(portset, tcp->th_dport);
	} else if (ip->ip_p == IPPROTO_UDP) {
		struct sniff_udp *udp = (struct sniff_udp*)(packet + SIZE_ETHERNET + size_ip);
		PortSet_set(portset, udp->uh_dport);
	} else {
		return;
	}

	if(portset->count > 100) {
		IP_PORT_SCAN_ATTACKER[0] = src[0];
		IP_PORT_SCAN_ATTACKER[1] = src[1];
		IP_PORT_SCAN_ATTACKER[2] = src[2];
		IP_PORT_SCAN_ATTACKER[3] = src[3];
		foundAttack = 1;
	}
}

void portScanAttackPrintPacketsCallback(u_char *user, const struct pcap_pkthdr *h, const u_char *packet) {
	struct sniff_ethernet *ethernet = (struct sniff_ethernet*)(packet);
	struct sniff_ip *ip = (struct sniff_ip*)(packet + SIZE_ETHERNET);
	u_int size_ip = IP_HL(ip)*4;
	if (size_ip < 20) {
	...
	// only look at packets from attacker to victim
	if (!compareMem(src, IP_PORT_SCAN_ATTACKER, 4) || !compareMem(dest, IP_PORT_SCAN_VICTIM, 4)) {
		return;
	}

	// Analysis	
	// printf("PACKET ID: (%d, %d\n)", ip->ip_id, ip->ip_sum, tcp->th_sum);
...
void detectPortScanAttack(char *filepath) {
	...
	filterPackets(filepath, "", portScanAttackCallback, (u_char *)table);
	if (foundAttack == 1) {
		printf("%s\n", "found port scan attack");
		printf("%s", "VICTIM: ");
		print_int_memory(IP_PORT_SCAN_VICTIM, 4);
		printf("%s", "ATTACKER: ");
		print_int_memory(IP_PORT_SCAN_ATTACKER, 4);
		printf("%s\n", "<PROTO> (<IP_ID>, <IP_SUM>, <PROTO_SUM>)");
		filterPackets(filepath, "", portScanAttackPrintPacketsCallback, NULL);
	} else {
		printf("%s\n", "no port scan attack found");
	}
	IPAddrHashTable_destroy(table, (ValueDestroyer)PortSet_destroy);
}
